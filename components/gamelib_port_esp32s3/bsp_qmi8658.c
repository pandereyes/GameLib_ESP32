...
{
    esp_err_t ret;
    ret = i2c_master_write_read_device(PORT_I2C_NUM, dev_addr, &reg_addr, 1, data, len, 1000 / portTICK_PERIOD_MS);
    return ret;
}
...
esp_err_t bsp_i2c_reg8_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint8_t len)
{
    ...
    ret = i2c_master_write_to_device(PORT_I2C_NUM, dev_addr, tx_data, len + 1, 1000 / portTICK_PERIOD_MS);
    free(tx_data);
    return ret;
}
...
void bsp_qmi8658_init(void)
{
    ...
    const i2c_config_t i2c_conf = {
        ...
        .sda_io_num = PORT_I2C_PIN_SDA,
        .scl_io_num = PORT_I2C_PIN_SCL,
        .sda_pullup_en = GPIO_PULLUP_ENABLE,
        .scl_pullup_en = GPIO_PULLUP_ENABLE,
        .master.clk_speed = 400000,
    };
    /* Initialize I2C */
    ESP_ERROR_CHECK(i2c_param_config(PORT_I2C_NUM, &i2c_conf));
    ESP_ERROR_CHECK(i2c_driver_install(PORT_I2C_NUM, i2c_conf.mode, 0, 0, 0));
